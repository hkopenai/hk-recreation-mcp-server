...
This module contains unit tests to verify the functionality of MCP tools
through HTTP communication with a running server.
"""
...
class TestMCPClientSimulation(unittest.TestCase):
    ...
    This class tests the functionality of MCP tools by communicating with a running server.
    """
    server_process = None
    ...
    def setUp(self):
        ...
        self.server_process = subprocess.Popen(
            [sys.executable, "-m", "hkopenai.hk_recreation_mcp_server", "--sse"],
    ...
    def test_get_creative_goods_trade_tool(self):
        """
        Test the 'get_creative_goods_trade' tool by calling it with a year range.
        Verifies that the response is a valid dictionary or list without errors.
        """
        logger.debug("Testing 'get_creative_goods_trade' tool...")
        asyncio.run(
            self._call_tool_and_assert(
                "get_creative_goods_trade", {"start_year": 2023, "end_year": 2026}
            )
        )
